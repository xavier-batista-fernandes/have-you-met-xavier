...
export default function Home() {
  ...
  return (
    <main
      className="relative flex h-screen justify-center py-[5%]"
    ...
    >
      ...
      <div className="relative h-full w-5/6 flex-[0_0_auto]">
        {cards.map((card) => {
          return (
            <div
              id={`card-${card.id}`}
      ...
      <ul className="my-[1%] flex flex-[0_0_auto] flex-col gap-2">
        {cards.map((card) => {
          const isCardActive =
            Math.max(...cards.map((c) => c.zIndex)) === card.zIndex
          return (
            <li
              id={`label-${card.id}`}
              key={card.id}
              className="h-[25%] overflow-hidden rounded-r-xl"
            >
              <button
                type="button"
                onClick={() => !isCardActive && handleCardClick(card.id)}
                className={`py-2 text-center font-extrabold select-none ${isCardActive ? 'cursor-default' : 'cursor-pointer'} h-full`}
                style={{
